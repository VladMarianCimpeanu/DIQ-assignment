...
def accuracy_assesment(imputed_df_s: list, original_df, numeric_columns=[], vector_columns=[]) -> list:
    """
    ...
    for i_df in imputed_df_s:
        ...
        for c in columns:
            ...
            if c in numeric_columns:
                ...
                distance_function = lambda x, y: np.abs((np.dot(np.array(x), np.array(y))))
            else:
